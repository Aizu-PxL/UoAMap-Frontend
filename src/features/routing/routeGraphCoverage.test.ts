...
describe("実生成Routeグラフcoverage", () => {
  ...
  test("生成グラフのノード・エッジ内訳を固定する", () => {
    expect(routeGraph.nodes.length).toEqual(348);
    expect(routeGraph.edges.length).toEqual(445);
    expect(routeGraph.edges.filter((edge) => edge.kind === "walk").length).toEqual(406);
    expect(routeGraph.edges.filter((edge) => edge.kind === "transfer").length).toEqual(39);
  });

  test("campus-all以外の全122 PlaceがRouteへ収録され講堂から到達可能", () => {
    ...
    expect(expectedPlaceIds.length).toEqual(122);
