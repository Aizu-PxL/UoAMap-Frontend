...
計画と受入ゲートは [PRODUCTION.md](PRODUCTION.md) を正とする。着手順:

1. 人間が本番origin、base path、ホスティング/ドメイン所有者、デプロイ承認者を確定
2. 人間とバックエンド担当がAPI本番origin、CORS/同一origin構成、データ更新権限を確定
3. 実装担当がAPI接続とホスティング設定を小さいブリーフへ分け、SPAフォールバック、Vite `base`、Router `basename`、API接続、デプロイ設定を実装
4. ステージングと本番でディープリンク・API・HTTPS・iPhone/Androidを受入
5. 人間が実印刷QRを現地条件で受入後、量産・設置・Go判断

## その他未決事項

SPEC §7 も参照(QR設置一覧とqrId採番、ホスティング先、バックエンド技術選定など)。公開関連の未決値は `docs/PRODUCTION.md` §3へ記録する。
