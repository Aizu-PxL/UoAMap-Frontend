...
- 抽出スクリプトが隣接フロアのペアごとに**transferエッジ**を自動生成する。SVGにフロアをまたぐpathは描かない
  ...
  - 距離はローカル距離`60`へ両フロア校正倍率の幾何平均を掛けたキャンパス相対コスト（`60 * sqrt(scaleA * scaleB)`）。短い廊下1本ぶん相当で、どの階段を選ぶかの比較にのみ効く
...
SVGごとにviewBox座標の縮尺が異なるため、抽出したwalk距離はビルド時にキャンパスSVG基準の単位へ校正する。node座標と`pathD`は各SVGのローカル座標を維持し、探索に使う`distance`だけを変換する。

- `campus`の倍率を1とする
- 親子フロアに共通する入口または階段transferの対応点を2点以上使い、全点対距離の最小二乗から子フロアの単一倍率を導出する
- 校正親は `rq-1f <- campus`、`rq-2f <- rq-1f`、`rq-3f <- rq-2f`、`lh-1f <- campus`、`lh-2f <- lh-1f`、`sh-1f <- campus`、`sh-2f <- campus`
- 単一倍率での正規化RMSEが15%を超える場合、対応点不足、ゼロ基線、未知の親、親循環がある場合は生成を失敗させる
- UBICとLICTiAは対応点が1入口しかないため倍率1のtopology-neutral fallbackを使う。該当フロアへ2本目の入口または階段を追加した時点でfallbackは失効し、対応点を増やして校正親を設定するまで生成を失敗させる
- 生成JSONの`distanceCalibration`へ倍率、親、使用transfer ID、正規化RMSEを保存する。Dijkstraはキャンパス相対単位へ校正済みの単一`distance`を従来どおり最小化し、屋内優先などの別ルールは持たない
...
- 距離は抽出時にノード座標間のユークリッド距離から生成し、そのフロアの校正倍率を掛けてキャンパス相対単位へ変換する
...
- 距離校正の不正（未知の親、親循環、対応点不足、ゼロ基線、正規化RMSE 15%超、fallbackフロアの入口・階段構成違反）
