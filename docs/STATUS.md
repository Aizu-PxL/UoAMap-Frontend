...
最終更新: 2026-07-21(講義棟1F〜2Fとキャンパス接続ルート03g完了)
...
| 3 | ルート | △ 一部 | 03a〜03g実装済み。研究棟16地点・講堂・学生ホール1Fの4地点・UBIC室内3地点と建物地点・講義棟1F〜2Fの9地点を接続。SH2Fの中央階段着地点も収録済み。LICTiA・ロボット格納庫は未実装 |
...
- **ルート**: `public/maps/` の `Route` グループを `scripts/extract-routes.ts` が `src/features/routing/generated/routeGraph.json` へ抽出する。作図契約は `docs/MAP_AUTHORING.md`。探索はフロントのDijkstra、描画はベースSVGとマーカーの間にある独立オーバーレイSVG。研究棟16地点・講堂・学生ホール1Fの4地点・UBIC室内3地点と建物地点・講義棟1F〜2Fの9地点を収録し、同じ`data-stair-id`を持つ隣接階ノード間へ固定コスト60、建物・キャンパス両側の同じ`data-entrance-id`間へコスト0のtransferエッジを生成する。1 SVGに複数フロアがある場合はRoute子要素ごとの`data-floor-id`で抽出する
