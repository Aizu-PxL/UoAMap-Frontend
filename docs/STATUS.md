...
最終更新: 2026-07-21(研究棟〜講堂のキャンパス横断ルート03cの実装・機械検証・ブラウザ確認完了)
...
| 3 | ルート | △ 一部 | 03a〜03c実装済み。研究棟1F〜3Fの16地点を階段・西側出入口・屋外歩行者路で講堂まで接続し、シート間transfer・フロア別描画・乗換マーカー・切替インジケータまで実装(幅402pxブラウザ確認済み)。他建物の室内接続は未実装 |
...
- **ルート**: `public/maps/` の `Route` グループを `scripts/extract-routes.ts` が `src/features/routing/generated/routeGraph.json` へ抽出する。作図契約は `docs/MAP_AUTHORING.md`。探索はフロントのDijkstra、描画はベースSVGとマーカーの間にある独立オーバーレイSVG。研究棟1F〜3Fの16地点と講堂を収録し、同じ`data-stair-id`を持つ隣接階ノード間へ固定コスト60、建物・キャンパス両側の同じ`data-entrance-id`間へコスト0のtransferエッジを生成する
