...
同一地点に第1回・第2回など複数イベントがある個別バッジは、開始時刻が現在時刻以降の`timeSlots`から最も早いイベントを代表として表示・遷移するようにした。複数スロットを持つ同一イベントの後続枠も候補に含め、全回の開始後または時刻情報なしではマーカーを維持して従来どおりデータ順の先頭へフォールバックする。マーカー生成時とタップ時に現在時刻を評価し、次の開始時刻直後にはバッジ自体を再生成するため、画面を開いたまま時刻境界をまたいでも表示名・`aria-label`・`data-event-*`・遷移先が同時に更新される。終了時刻がない回へ架空の終了時刻は補っていない。

`bun run verify:all`は168 tests / 1,118 assertions、production build、125 Place / 89 QR / 61 Event、350 nodes / 448 edges、git diff checkをPASSした。時刻注入テストで入力順に依存しない最早未実施回、第1回開始後の第2回、同一イベントの後続スロット、全回開始後のフォールバック、次の開始時刻直後の再生成時刻を確認した。

402×874pxでは、開催日前の時刻で講堂前の2件集約バッジから`A1`「大学説明会（1回目）」詳細へ遷移し、BottomSheet 82svh（716.68 / 874px）、既存`source=browser`クエリ保持、console error 0件を確認した。

初回の会話履歴なし読み取り専用レビューでは、画面を開いたまま時刻境界をまたいだ際にバッジの`aria-label`と`data-event-*`が古い代表回のまま残るP3が見つかった。開始時刻直後のバッジ再生成で解消し、全検証を再実行した。

修正後の最終読み取り専用再レビューは指摘なし。
