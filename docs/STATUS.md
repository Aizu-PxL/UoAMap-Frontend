...
最終更新: 2026-07-20(マップオーバーレイ横長固定サイズ補正07完了時点)
...
| 3 | ルート | △ 一部 | 03a完了。作図規約・SVG抽出・Dijkstra・ルート線描画を研究棟1Fの5地点で実装。階段・屋外・別建物またぎは未実装 |
...
- **ルート**: `public/maps/` の `Route` グループを `scripts/extract-routes.ts` が `src/features/routing/generated/routeGraph.json` へ抽出する。作図契約は `docs/MAP_AUTHORING.md`。探索はフロントのDijkstra、描画はベースSVGとマーカーの間にある独立オーバーレイSVG。03a時点の収録範囲は研究棟1Fの5地点のみ
